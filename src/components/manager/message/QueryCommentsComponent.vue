  ...
  <el-main>
    <div class="main">
        ...
        <div class="select-content">
          <el-form :model="form" label-width="80px">
            <div class="float">
              ...
              <el-form-item label="" width="200px" style="">
                <el-select
                ...
                >
                  <el-option label="" value="" />
                  <el-option label="积极" value="2" />
                  <el-option label="消极" value="0" />
                  <el-option label="中性" value="1" />
                </el-select>
              </el-form-item>
            </div>
            <div class="float2">
              ...
              <el-form-item
                label="开始时间"
                :style="{ display: form.dateType === 'scope' ? '' : 'none' }"
              >
                <el-col :span="11">
                  <el-date-picker
              ...
              <el-form-item
                label="某天时间"
                :style="{ display: form.dateType === 'day' ? '' : 'none' }"
              >
                <el-col :span="11">
                  <el-date-picker
                    v-model="form.someTime"
                    type="date"
                    placeholder="Pick a date"
                    style="width: 200px"
                  />
              ...
              <el-form-item
                label="开始时间"
                :style="{ display: form.dateType === 'time' ? '' : 'none' }"
              >
                <el-col :span="11">
                  <el-date-picker
                    v-model="form.hithertoTime"
                    type="date"
                    placeholder="Pick a date"
                    style="width: 200px"
                  />
      ...
      <div class="table">
        <div class="table-content">
          ...
          >
            ...
            >
              <template #default="{ row }">
                <el-tag
                  ...
                  "
                  disable-transitions
                  >{{ row.emotion == '2' ? "积极" : row.emotion == '0' ? "消极" :"中性" }}</el-tag
                >
              </template>
            </el-table-column>
            ...
            >
              <template #default="{ row }">
                <el-button type="primary" size="small" @click="collect(row.id)"
                  >收藏</el-button
                >
                <el-button type="info" size="small" @click="edit(row)"
                  >纠错</el-button
                >
                <el-button
                :type="row.type == 1? 'success' :'warning'"
                  size="small"
                  @click="earlyWarning(row.id, row.type)"
                  >{{ row.type == 1 ? "消警" : "预警" }}</el-button
                >
        ...
        <div class="page-content">
          <div class="demo-pagination-block">
            <el-pagination
              :style="{ display: page.total === 0 ? 'none' : '' }"
      ...
      <div class="edit" v-if="editVisible">
                <el-dialog v-model="editVisible" title="情感纠错" width="30%">
                    <el-form class="user" label-width="100px">
                        <el-form-item label="情感类型">
                            <el-select v-model="type_form.type" placeholder="" style="width: 200px" :disabled="!isEdit">
                                <el-option label="积极" value="2" />
                                <el-option label="中性" value="1" />
                                <el-option label="消极" value="0" />
                            </el-select>
                        ...
                        <el-form-item>
                            ...
                            <el-button v-if="isEdit" type="primary" @click="pushReset">提交</el-button>
...
function queryComment(pager, size) {
  ...
  if (
    null != form.startTime1 &&
    "" !== form.startTime1 &&
    null != form.startTime2 &&
    "" !== form.startTime2
  ) {
    ...
    endStart.value =
      form.startTime1.toDateString() + " " + form.startTime1.toTimeString();
  }
  ...
  if (
    ...
    endEnd.value =
      form.endTime1.toDateString() + " " + form.endTime1.toTimeString();
  }
...
function remove(commentId) {
  api.postDeleteComment(commentId).then((Response) => {
    if (true === Response.data.data.isDelete) {
      ...
      queryComment(page.currentPage, page.pageSize);
    } else {
      ElNotification({
        message: "删除失败",
        type: "error",
...
function earlyWarning(commentId, type) {
  api.postEarlyWarning(commentId).then((Response) => {
    if (true === Response.data.data.isTo) {
      if (type == 0) {
        ElNotification({
          message: "预警成功",
          type: "success",
...
function pushReset() {
  api.postResetEarlyWarning(type_form.id, type_form.type).then((Response) => {
    if (true === Response.data.data.isReset) {
      ElNotification({
        message: "纠错成功",
        type: "success",
        position: "bottom-left",
