                    ...
                    <el-form :model="form" label-width="80px">
                        <div class="float">
                            ...
                            <el-form-item label="" width="200px" style="">
                                <el-select v-model="form.emotion" placeholder="选择情感类型" style="width: 200px">
                                    <el-option label="" value="" />
                ...
                <div class="table-content">
                    <el-table :data="tableData" height="720" border highlight-current-row stripe style="width: 100%"
                        ...
                        <el-table-column prop="emotion" label="情感" width="100px" align="center" :resizable="false">
                            <template #default="{ row }">
                                <el-tag :type="row.emotion == '2'? 'success': row.emotion == '0'? 'danger': 'primary'" disable-transitions>
                                    {{ row.emotion == '2' ? "积极" : row.emotion == '0' ? "消极" :"中性" }}
                                </el-tag>
                            </template>
                        </el-table-column>
                        ...
                        <el-table-column label="操作" width="100px" align="center" :resizable="false">
                            <template #default="{ row }">
                                <el-button type="primary" size="small" @click="cancelCollect(row.id)">删除</el-button>
                            </template>
                        </el-table-column>
                    </el-table>
                </div>
...
function queryComment(pager, size) {
    ...
    api.postCollectCommentList(pager, size, store.state.user.id, form.keyword, form.area, form.url, form.sort, form.emotion, form.dateType,
...
function cancelCollect(comId) {
    api.postCancelCollect(store.state.user.id, comId).then(Response => {
        if (true === Response.data.data.isCancel) {
            ElNotification({
                message: '取消收藏',
                type: 'success',
